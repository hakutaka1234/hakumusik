...
from HakuRobotXMusic import YouTube, app
from HakuRobotXMusic.core.call import Haku
from HakuRobotXMusic.misc import SUDOERS, db
from HakuRobotXMusic.utils.database import (is_active_chat,
                                       is_music_playing, set_loop,
                                       is_nonadmin_chat, music_off,
                                       music_on)
from HakuRobotXMusic.utils.decorators.language import languageCB
from HakuRobotXMusic.utils.formatters import seconds_to_min
from HakuRobotXMusic.utils.inline.play import (panel_markup_1,
                                          stream_markup,
                                          telegram_markup)
from HakuRobotXMusic.utils.stream.autoclear import auto_clean
from HakuRobotXMusic.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await Haku.pause_stream(chat_id)
        ...
        await Haku.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
            _["admin_4"].format(mention)
        )
    elif command == "Stop" or command == "End":
        await CallbackQuery.answer()
        await Haku.stop_stream(chat_id)
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await Haku.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                await Haku.skip_stream(chat_id, link, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            try:
                await Haku.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
